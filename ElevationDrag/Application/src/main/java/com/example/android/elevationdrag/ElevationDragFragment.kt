...
class ElevationDragFragment : Fragment() {
    ...
    /**
     * First we call our super's implementation of `onCreate`. Then we initialize our field
     * `ViewOutlineProvider mOutlineProviderCircle` with a new instance. Finally we initialize
     * `mElevationStep` with the raw pixel size that a Resources instance for the application's
     * package resolves for the resource with id R.dimen.elevation_step.
     *
     ...
     * by using our parameter `LayoutInflater inflater` to inflate our layout file R.layout.ztranslation
     * using our parameter `ViewGroup container` for the LayoutParams without attaching to it.
     * We initialize `View floatingShape` by finding the view in `rootView` with id R.id.circle,
     * set its `ViewOutlineProvider` to our field `ViewOutlineProvider mOutlineProviderCircle`
     * (generates the Outline that defines the shape of the shadow it casts, and enables outline clipping),
     * and call its `setClipToOutline(true)` method to indicate that the View's Outline should be used
     * to clip the contents of the View.
     *
     *
     * We initialize `DragFrameLayout dragLayout` by finding the view in `rootView` with id
     * R.id.main_layout and set its `DragFrameLayoutController` to an anonymous class whose `onDragDrop`
     * override animates the `translationZ` of `floatingShape` depending on whether its
     * `captured` parameter is true (view is raised by 50 pixels while captured), or false (translationZ
     * is returned to 0 when dropped). We then add `floatingShape` to the list of views that are draggable
     * within the container `dragLayout`.
     *
     *
     * We set the `OnClickListener` of the view in `rootView` with id R.id.raise_bt ("Z+")
     * to an anonymous class whose `onClick` override adds `mElevationStep` to `mElevation`,
     * logs the new elevation, and sets the base elevation of `floatingShape` to `mElevation`,
     * and we set the `OnClickListener` of the view in `rootView` with id R.id.lower_bt ("Z-")
     * to an anonymous class whose `onClick` override subtracts `mElevationStep` from
     * `mElevation` (setting to 0 if the result is less than 0), logs the new elevation, and sets
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        /* Raise the circle in z when the "z+" button is clicked. */rootView.findViewById<View>(R.id.raise_bt).setOnClickListener {
        ...
        /* Lower the circle in z when the "z-" button is clicked. */rootView.findViewById<View>(R.id.lower_bt).setOnClickListener {
            mElevation -= mElevationStep.toFloat()
    ...
    private class CircleOutlineProvider : ViewOutlineProvider() {
         ...
         * an alpha of `1.0f`. We call the `setOval` method of our parameter
         * `Outline outline` to set the outline to the oval defined by input rect given by a
         * left side at 0, a top at 0, the width of our parameter `View view` as the right side,
         * and the height of our parameter `View view` as the bottom side.
         *
