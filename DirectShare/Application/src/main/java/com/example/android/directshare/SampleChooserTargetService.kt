...
class SampleChooserTargetService : ChooserTargetService() {
    /**
     * Called by the system to retrieve a set of deep-link [targets][ChooserTarget] that
     ...
     *
     *
     * First we create `ComponentName componentName` from the name of this application's package
     * and the canonical name of the class `SendMessageActivity`. Then we initialize our variable
     * `ArrayList<ChooserTarget> targets` with a new instance. Then indexed by `i` we loop
     * through all the `Contact` objects in the array `Contact[] CONTACTS` setting
     * `Contact contact` to the `Contact` with id `i`, initializing `Bundle extra`
     * with a new instance then adding `i` as an extra to it under the key Contact.ID ("contact_id").
     * We then add a new `ChooserTarget` to the list `targets` constructed using the name
     * of `contact` as the title of the target that will be shown to a user, an Icon pointing to
     * the drawable resource of the icon of `contact` as the icon to represent the target, a ranking
     * score of 0.5, `componentName` as the component to be launched if this target is chosen, and
     * `extras` as extra values to be merged into the Intent when this target is chosen.
     *
     *
    ...
    override fun onGetChooserTargets(targetActivityName: ComponentName,
                                     matchedFilter: IntentFilter): List<ChooserTarget> {
        val componentName = ComponentName(packageName,
            SendMessageActivity::class.java.canonicalName!!)
        ...
        for (i in Contact.CONTACTS.indices) {
            ...
            targets.add(ChooserTarget( // The name of this target.
                contact.name,  // The icon to represent this target.
                Icon.createWithResource(this, contact.icon),  // The ranking score for this target (0.0-1.0); the system will omit items with
                ...
                componentName,  // The extra values here will be merged into the Intent when this target is
                // chosen.
                extras))
        }
        return targets
