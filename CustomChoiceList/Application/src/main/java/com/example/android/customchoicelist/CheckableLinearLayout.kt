...
(context: Context?, attrs: AttributeSet?) : LinearLayout(context, attrs), Checkable {
    ...
    /**
     * Part of the `Checkable` interface, we just return the value of our `boolean mChecked`
    ...
    /**
     * Change the checked state of the view. If our parameter `b` is not equal to our field
     * `boolean mChecked` we set `mChecked` to it then call the `refreshDrawableState`
     * method to force our view to update its drawable state. This will cause drawableStateChanged to
     * be called on this view. Part of the `Checkable` interface.
     *
     ...
     * `setChecked` with the inverse of our field `mChecked`. Part of the `Checkable`
     * interface.
     */
    override fun toggle() {
        isChecked = !mChecked
     ...
     * variable `int[] drawableState` with the array holding the current `Drawable`
     * state of the view requesting one more entry than our parameter `int extraSpace` has
     * requested using our super's `onCreateDrawableState` method. If our method `isChecked`
     * returns true (our view is checked) we call the `mergeDrawableStates` method to merge our
     * additional state values in `int[] CHECKED_STATE_SET` into `drawableState`. In any
