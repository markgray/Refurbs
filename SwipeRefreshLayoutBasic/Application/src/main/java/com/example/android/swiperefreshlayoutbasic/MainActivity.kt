...
class MainActivity : SampleActivityBase() {
     ...
     * [SwipeRefreshLayoutBasicFragment] variable `val fragment` with a new instance and use
     * `transaction` to replace (add) `fragment` to the container view with ID
     ...
     * [SwipeRefreshLayoutBasicFragment] fragment, if not null we are being recreated after a
     * configuration change and the system will take care of restoring the fragment.
     */
     ...
     * `R.id.sample_output` is an instance of [ViewAnimator] (screen is narrower than 720dp),
     ...
     * [MenuItem.getItemId] method (kotlin `itemId` property) of our [MenuItem] parameter [item] is
     * `R.id.menu_toggle_log` we toggle the value of [Boolean] flag field [mLogShown], then
     * initialize [ViewAnimator] variable `val output` by finding the view with ID
     * `R.id.sample_output`. If [mLogShown] is now `true` we set the displayed child of `output` to
     * 1, if it is `false` we we set the displayed child of `output` to 0. We then call the method
     * `invalidateOptionsMenu` to declare that the options menu has changed, so should be recreated,
     * and return `true` to the caller to consume the event here. If the item id is not
