...
class AssetProvider : ContentProvider() {
     ...
     * return sub-sections of files, often assets inside of their .apk. We initialize our variable
     * `String assetName` with the last segment in the path of `Uri uri`, then if this
     * is the empty string we throw FileNotFoundException. Otherwise, wrapped in a try block intended
     * to catch IOException we initialize `AssetManager am` with an AssetManager instance for
     * the application's package, and use it to open a file descriptor for `assetName` which
