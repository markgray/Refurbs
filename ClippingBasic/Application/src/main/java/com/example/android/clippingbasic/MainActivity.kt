...
class MainActivity : SampleActivityBase() {
     ...
     * `R.id.sample_main_layout` then call [ViewCompat.setOnApplyWindowInsetsListener] to
     ...
     * [ClippingBasicFragment] variable `val fragment` with a new instance and use `transaction`
     ...
     * [ClippingBasicFragment] fragment, if not `null` we are being recreated after
     ...
     * the value of [mLogShown], then initialize [ViewAnimator] variable `val output` by finding the
     * view with ID `R.id.sample_output`. If [mLogShown] is `true` we set the displayed child of
     * `output` to 1, if it is false we set the displayed child of `output` to 0. We then call
