...
class MainActivity : AppCompatActivity(), ActionBar.TabListener {
     ...
     * [ViewCompat.setOnApplyWindowInsetsListener] to take over over
     * the policy for applying window insets to `rootView`, with the
     * `listener` argument a lambda that accepts the [View] passed the
     * lambda in variable `v` and the [WindowInsetsCompat] passed the lambda
