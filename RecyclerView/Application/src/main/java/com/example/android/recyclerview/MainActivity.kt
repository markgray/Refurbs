...
class MainActivity : SampleActivityBase() {
     ...
     * [RecyclerViewFragment] variable `val fragment` with a new instance and use `transaction`
     ...
     *
     * @param savedInstanceState If this is `null` we need to create and add our [RecyclerViewFragment]
     ...
     * [MenuItem] parameter [item] with id `R.id.menu_toggle_log` we toggle the value of [mLogShown],
     * then initialize [ViewAnimator] variable `val output` by finding the view with ID
     * `R.id.sample_output`. If `mLogShown` is `true` we set the displayed child of `output` to 1,
     * if it is `false` we we set the displayed child of `output` to 0. We then call the method
     * [invalidateOptionsMenu] to declare that the options menu has changed, so should be recreated,
     * and return `true` to the caller to consume the event here. If the ID of [item] is not
