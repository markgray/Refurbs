...
class ImageGridActivity : FragmentActivity() {
     ...
     * using android.os.StrictMode. Then we call our super's implementation of `onCreate`.
     *
     *
     * If the `FragmentManager` for interacting with fragments associated with this activity
     * can not find a fragment with the tag `TAG`, we initialize `FragmentTransaction ft`
     * by starting a series of edit operations on the Fragments associated with this FragmentManager,
     ...
     * We then call the `commit` method of `ft` to schedule a commit of this transaction.
     *
