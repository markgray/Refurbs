...
class TouchDisplayView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
     ...
     * pointer (ACTION_DOWN) has already been received) We initialize `int index` with
     * the associated pointer index of `event`, and `int id` with the pointer
     ...
     * the ACTION_POINTER_DOWN action is stored). We then initialize `TouchHistory data`
     * with an instance (from the pool, or new) which has its x, y, and pressure components set
     * to the values of the x, y, and pressure components from pointer index `index` of
     * `event`. We then set the `label` field of `data` to a string formed
     * by concatenating the string value of `id` to the string "id: ". We then store
     * `data` under the index `id` to `SparseArray<TouchHistory> mTouches`,
     * and break.
     *
     *  *
     * ACTION_UP: (Final pointer has gone up and has ended the last pressed gesture) We retrieve
     * the pointer identifier associated with this event from pointer index 0 (it will be the
     * only event stored in the MotionEvent object) of `event` in order to initialize
     * `int id`. We initialize `TouchHistory data` with the `TouchHistory`
     * object stored under key `id` in `mTouches`, remove that entry from `mTouches`
     * and then recycle `data`. We then set `mHasTouch` to false, and break.
     *
     *  *
     * ACTION_POINTER_UP: (A non-primary pointer has gone up and other pointers are still active)
     * We initialize `int index` with the associated pointer index of `event`, and
     * `int id` with the pointer identifier associated the pointer data index `index`
     ...
     * `TouchHistory data` with the `TouchHistory` object stored under key `id`
     * in `mTouches`, remove that entry from `mTouches`, recycle `data`, then
     * break.
     *
     *  *
     * ACTION_MOVE: (A change event happened during a pressed gesture. (Between ACTION_DOWN
     * and ACTION_UP or ACTION_POINTER_DOWN and ACTION_POINTER_UP)) We loop over `index`
     * from 0 for the number of pointers of data contained in `event`, initializing `id`
     * to the pointer identifier associated with the pointer data index `index`. We initialize
     * `TouchHistory data` by fetching the object stored at `id` in our field
     * `SparseArray<TouchHistory> mTouches`. We then add the previous X and Y coordinates
     * of `data` (the `x` and `y` fields) to the history of `data`.
     * Finally we update the `x`, `y`, and `pressure` fields of `data`
     * with the X, Y, and pressure values for `event` for the pointer index `index`
     * before looping back for the next pointer index. When done with all the pointer indices
     * we break.
     *
     *
     * When done dealing with the `MotionEvent` we invalidate ourselves to trigger redraw on UI
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (action and MotionEvent.ACTION_MASK) {
            ...
            MotionEvent.ACTION_POINTER_DOWN -> {
                ...
                val data = TouchHistory.obtain(event.getX(index), event.getY(index),
    ...
    /**
     * Implement this to do your drawing. First we call our super's implementation of `onDraw`.
     * If our field `mHasTouch` is true (at least one finger is touching the screen) we fill the
     * entire canvas' bitmap with the color BACKGROUND_ACTIVE (WHITE), if false (no fingers touching)
     * We draw a rectangle using `Paint mBorderPaint` around the entire canvas. `mBorderPaint`
     * is a INACTIVE_BORDER_COLOR (0xFFffd060) colored line (a light-ish orange) whose width is
     * `mBorderWidth` pixels and the size of the rectangle is inset by `mBorderWidth`
     * pixels so the result is a frame around the entire canvas.
     *
     *
     * Next we loop over `int i` for all the entries in `SparseArray<TouchHistory> mTouches`,
     * setting `int id` to the pointer id of the key of the i'th key-value mapping that the
     * SparseArray stores, and setting `TouchHistory data` to the value from the i'th key-value
     * mapping that the SparseArray stores. We then call our method `drawCircle` to draw the data
     * encapsulated by `TouchHistory data` to `canvas` using the color index `id`.
     *
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (mHasTouch) {
            ...
            canvas.drawRect(mBorderWidth, mBorderWidth, width - mBorderWidth, height
        ...
        for (i in 0 until mTouches.size()) {
            ...
            val id = mTouches.keyAt(i)
            val data = mTouches.valueAt(i)
    ...
    /**
     * Sets up the required [android.graphics.Paint] objects for the screen density of this
     * device. We initialize `float density` with the logical density of the display (a scaling
     * factor for the Density Independent Pixel unit, where one DIP is one pixel on an approximately
     * 160 dpi screen). We use `density` to scale CIRCLE_RADIUS_DP to `mCircleRadius`,
     * and to scale CIRCLE_HISTORICAL_RADIUS_DP to `mCircleHistoricalRadius`. We set the text
     * size of `Paint mTextPaint` to 27f and its color to BLACK. We use `density` to scale
     * INACTIVE_BORDER_DP to `mBorderWidth`, set the stroke width of `Paint mBorderPaint`
     * to `mBorderWidth`, set its color to INACTIVE_BORDER_COLOR (a light-ish orange), and set
     * its style to STROKE (Geometry and text drawn with this style will be stroked, respecting the
     * stroke-related fields on the paint without filling, so that the text of the `TextView`
     * we share the `FrameLayout` with shows).
     */
    ...
    /**
     * Draws the data encapsulated by a [TouchDisplayView.TouchHistory] object to a canvas.
     * A large circle indicates the current position held by the
     * [TouchDisplayView.TouchHistory] object, while a smaller circle is drawn for each
     * entry in its history. The size of the large circle is scaled depending on
     * its pressure, clamped to a maximum of `1.0`.
     *
     *
     * We initialize `int color` by selecting a color based on the id (our parameter
     * `id` modulo the length of `COLORS` is used as an index into `COLORS`).
     * We initialize `float pressure` to be the minimum of the `pressure` field of
     * `data` and 1.0f, and initialize `float radius` to `mCircleRadius` multiplied
     * by `pressure`. We then draw a circle on `Canvas canvas` using `mCirclePaint`
     * of radius `radius` using the `x` field for the x coordinate of the center of the
     * circle, and the `y` field shifted by half `radius` up for the y coordinate of the
     * center (no idea why). We then set the alpha value of `mCirclePaint` to 125 to draw all
     * historical points with a lower alpha value. We then loop over `int j` for all of the
     * `PointF` entries in the `PointF[] history` field of `data` (the
     * `historyCount` field of `data` is the number currently in the array, but for extra
     * safety we also limit ourselves to the total length of the `history` field). We initialize
     * `PointF p` with the entry in `data.history[j]` and draw a circle on `canvas`
     * using `Paint mCirclePaint` of radius `mCircleHistoricalRadius` located using the
     * `x` field of `p` for the x coordinate and the `y` field of `p` for the
     * y coordinate.
     *
     *
     * When done drawing the history circles we draw text using `Paint mTextPaint` with the
     * `label` field as the text located using the `x` field of `data` offset to
     * the right by `radius` for the x coordinate, and the `y` field of `data`
     * offset up by `radius`.
     *
    ...
    protected fun drawCircle(canvas: Canvas, id: Int, data: TouchHistory) {
         ...
         */
        val pressure = Math.min(data.pressure, 1f)
        val radius = pressure * mCircleRadius
