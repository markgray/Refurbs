...
class GenericAccountService : Service() {
    /**
     * Our instance of `Authenticator` (a `AbstractAccountAuthenticator` implementation)
     */
     ...
     * initialize our field `Authenticator mAuthenticator` with a new instance.
     */
     ...
     * the service. We return the `IBinder` returned by the `getIBinder` method of our
     * field `Authenticator mAuthenticator`.
     *
    ...
    (context: Context?) : AbstractAccountAuthenticator(context) {
         ...
         * is set in the bundle then this response field is to be used for sending
        ...
        /**
         * Update the locally stored credentials for an account. We just throw UnsupportedOperationException
         *
         * @param response      to send the result back to the AccountManager, will never be null
    ...
    companion object {
        ...
        /**
         * Account name we are using (Normally user's identity, but we are not using user accounts so
         * any string will do).
         */
