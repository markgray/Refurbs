...
/**
 * Service to handle sync requests. It is declared as such in the AndroidManifest.xml with a
...
class SyncService : Service() {
     ...
     * constructor and log "Service created". Then we synchronize on `sSyncAdapterLock` and
     * if our field `SyncAdapter sSyncAdapter` is null we initialize it with a new instance
     ...
     * `ContentResolver.setIsSyncable(android.accounts.Account, String, int)` with 1 if it is
     * currently set to <0).
     */
    ...
    /**
     * Return Binder handle for IPC communication with [SyncAdapter]. New sync requests will
     * be sent directly to the SyncAdapter using this channel. We just return the result of calling
     * the `getSyncAdapterBinder` method of our field `SyncAdapter sSyncAdapter` (which
    ...
    companion object {
        ...
        /**
         * Our instance of `SyncAdapter` which we use to retrieve its `IBinder` to return when
         * our `onBind` method is called.
         */
