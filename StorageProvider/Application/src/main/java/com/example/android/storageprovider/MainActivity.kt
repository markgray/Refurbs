...
class MainActivity : SampleActivityBase() {
     ...
     * unable to find a fragment with the tag [FRAGTAG], we initialize [FragmentTransaction]
     * variable `val transaction` by using the [FragmentManager.beginTransaction] method to begin
     * it, create a new instance for [StorageProviderFragment] variable `val fragment`, use
     * `transaction`'s [FragmentTransaction.add] method to add `fragment` to `transaction` with
     * the tag [FRAGTAG] and then use `transaction`'s [FragmentTransaction.commit] method to commit
     * `transaction`.
     *
    ...
    companion object {
        ...
        /**
         * Fragment tag for our `StorageProviderFragment` fragment.
         */
