...
/**
 * A sample which shows how to use [SwipeRefreshLayout] within a
 * [ListFragment] to add the 'swipe-to-refresh' gesture to a
 * [android.widget.ListView]. This is provided through the provided re-usable
 ...
 * [SwipeRefreshLayout] through the options menu. This is meant to
...
class SwipeRefreshListFragmentFragment : SwipeRefreshListFragment() {
     ...
     * then we call the `setHasOptionsMenu(true)` method to notify the system that this fragment
     * would like to participate in populating the options menu by receiving a call to [.onCreateOptionsMenu]
     * and related methods.
     *
     ...
     * of `onViewCreated`. We initialize `ListAdapter adapter` with a new `ArrayAdapter`
     * constructed from the LIST_ITEM_COUNT (20) random cheeses selected by the `randomList` method
     * of `Cheeses` using our `FragmentActivity` for `Context`, the layout file with
     * resource id android.R.layout.simple_list_item_1 to use when instantiating views, and the id
     * android.R.id.text1 for the TextView within the layout resource to be populated. We then set the
     * list adapter of our `ListView` to `adapter`. Finally we call the `setOnRefreshListener`
     * method of our `SwipeRefreshListFragment` super to set its `OnRefreshListener` to an
     * anonymous class whose `onRefresh` override calls our `initiateRefresh` method to
     * initiate a background refresh of our list of random cheeses.
     *
     ...
     * `MenuInflater inflater` to inflate our menu layout file R.menu.main_menu into our parameter
     * `Menu menu`.
     *
     ...
     * `isRefreshing` returns false (we are not already refreshing) we call its method
     * `setRefreshing(true)` to start the SwipeRefreshLayout progress bar. We then call
     * our `initiateRefresh` method to start our refresh background task and return true
     * to consume the event here.
     *
     *  *
     * R.id.menu_color_scheme_1: we log the fact that we were selected, call the `setChecked(true)`
     * method of our parameter `MenuItem item` to have it display a checked mark, call
     * our super's `setColorScheme` method with four resource ids for colors for it to
     * use when displaying the SwipeRefreshLayout progress bar, and return true to consume
     * the event here.
     *
     *  *
     * R.id.menu_color_scheme_2: we log the fact that we were selected, call the `setChecked(true)`
     * method of our parameter `MenuItem item` to have it display a checked mark, call
     * our super's `setColorScheme` method with four resource ids for colors for it to
     * use when displaying the SwipeRefreshLayout progress bar, and return true to consume
     * the event here.
     *
     *  *
     * R.id.menu_color_scheme_3: we log the fact that we were selected, call the `setChecked(true)`
     * method of our parameter `MenuItem item` to have it display a checked mark, call
     * our super's `setColorScheme` method with four resource ids for colors for it to
     * use when displaying the SwipeRefreshLayout progress bar, and return true to consume
     * the event here.
     *
     *
     * If the item id is not one we know about, we return the value returned by our super's implementation
     ...
     * We log the fact that we were called, then call the `execute()` method of a new instance
     * of `DummyBackgroundTask` to load our list of random cheeses in a background AsyncTask
     * thread.
     */
     ...
     * initialize our variable `ArrayAdapter<String> adapter` with a reference to the
     * ListAdapter associated with this fragment's ListView, and call its `clear` method to
     * remove all elements from its data set. We then loop through all of the `String cheese`
     * in our parameter `List<String> result` adding each `cheese` to `adapter`.
     * Finally we call the `setRefreshing(false)` method of our super to stop the refreshing
     * progress indicator.
     *
     * @param result List of 20 random cheeses to use as the data set for our `ListAdapter`.
     */
    ...
    private inner class DummyBackgroundTask : AsyncTask<Void?, Void?, List<String?>?>() {
         ...
         * (none in our case). Wrapped in a try block intended to catch and log InterruptedException
         * we sleep for TASK_DURATION (3000ms) then we return the list of LIST_ITEM_COUNT (20) random
         * cheeses generated by the `Cheeses.randomList` method.
         *
