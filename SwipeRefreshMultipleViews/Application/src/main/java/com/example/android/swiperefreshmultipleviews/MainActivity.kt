...
class MainActivity : SampleActivityBase() {
     ...
     * [SwipeRefreshMultipleViewsFragment] variable `val fragment` with a new instance and use
     ...
     * [SwipeRefreshMultipleViewsFragment] fragment, if not `null` we are being
     * recreated after a configuration change so the system will take care of restoring
     * the fragment.
     */
     ...
     * `R.id.sample_output` is an instance of [ViewAnimator], and set its title to `R.string.sample_hide_log`
     * ("Hide Log") if our [Boolean] flag field [mLogShown] is `true` or to `R.string.sample_show_log`
     ...
     * the value of [Boolean] flag field [mLogShown], then initialize [ViewAnimator] variable
     * `val output` by finding the view with ID `R.id.sample_output`. If [mLogShown] is now `true`
     * we set the displayed child of `output` to 1, if it is `false` we we set the displayed child
     * of `output` to 0. We then call the method [invalidateOptionsMenu] to declare that the options
