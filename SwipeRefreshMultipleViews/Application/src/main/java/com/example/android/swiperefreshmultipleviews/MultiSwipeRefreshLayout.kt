...
class MultiSwipeRefreshLayout : SwipeRefreshLayout {
    /**
     * The views we implement `SwipeRefreshLayout` for.
     */
     ...
     * views need to be a descendant of this view. We allocate enough storage to hold our parameters
     * `int... ids` for our field `View[] mSwipeableChildren`. Then we loop over `i`
     * for all the int's in `ids` setting `mSwipeableChildren[ i]` by finding the view in
     * our view with id `ids[ i]`.
     *
     ...
     *
     *
     * If our field `View[] mSwipeableChildren` is not null and has more than 0 entries, we
     * loop through each of the `View view` in `mSwipeableChildren` and if `view`
     * is not null and the view and all of its ancestors are [.VISIBLE], and our method
     * `canViewScrollUp` determines that the view cannot scroll up we return false to start the
     * refresh gesture. Otherwise we return true.
     *
    ...
    companion object {
         ...
         * We return the value returned by the `canScrollVertically` method of our parameter
         * `View view` for the direction -1 (negative to check scrolling up).
         *
         * @param view Whether it is possible for the `View view` to scroll up.
         */
