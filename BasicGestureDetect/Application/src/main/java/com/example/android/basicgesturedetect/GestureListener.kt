...
class GestureListener : SimpleOnGestureListener() {
     ...
     * `getTouchType` generates for our parameter `MotionEvent e`. We then return false
     ...
     * string that our method `getTouchType` generates for our parameter `MotionEvent e`.
     *
     ...
     * move [MotionEvent]. The distance in x and y since the last call to `onScroll` is
     * also supplied for convenience. We log the message formed by concatenating the string "Scroll"
     * with the touch type string that our method `getTouchType` generates for our parameter
     * `MotionEvent e1`. We then return false to allow the event to propagate to the underlying
     * view.
     *
     ...
     * touch type string that our method `getTouchType` generates for our parameter `MotionEvent e1`.
     * We then return false to allow the event to propagate to the underlying view.
     *
    ...
    /**
     * The user has performed a down [MotionEvent] and not performed a move or up yet. This
     * event is commonly used to provide visual feedback to the user to let them know that their
     * action has been recognized i.e. highlight an element. We log the message formed by concatenating
     * the string "Show Press" with the touch type string that our method `getTouchType` generates
     * for our parameter `MotionEvent e`.
     *
     ...
     * method `getTouchType` generates for our parameter `MotionEvent e`. We then return
     * false to allow the event to propagate to the underlying view.
     *
     * @param e The down motion event.
     * @return true if the event is consumed, else false
     */
     ...
     * "Double tap" with the touch type string that our method `getTouchType` generates for
     * our parameter `MotionEvent e`. We then return false to allow the event to propagate
     ...
     * the touch type string that our method `getTouchType` generates for our parameter
     * `MotionEvent e`. We then return false to allow the event to propagate to the underlying
     * view.
     *
    ...
    /**
     * Notified when a single-tap occurs. Unlike `onSingleTapUp(MotionEvent)`, this will only
     * be called after the detector is confident that the user's first tap is not followed by a second
     * tap leading to a double-tap gesture. We log the message formed by concatenating the string
     * "Single tap confirmed" with  the touch type string that our method `getTouchType`
     * generates for our parameter `MotionEvent e`. We then return false to allow the event to
     * propagate to the underlying view.
     *
    ...
    companion object {
        ...
        /**
         * Returns a human-readable string describing the type of touch that triggered a MotionEvent. We
         * initialize our variable `String touchTypeDescription` to the string " ", and initialize
         * our variable `int touchType` with the tool type of pointer index 0 (this can be
         * TOOL_TYPE_UNKNOWN, TOOL_TYPE_FINGER, TOOL_TYPE_STYLUS, TOOL_TYPE_ERASER or TOOL_TYPE_MOUSE).
         * We then switch on `touchType`:
         *
         *  *
         * TOOL_TYPE_FINGER: we append the string "(finger)" to `touchTypeDescription` and
         * break.
         *
         *  *
         * TOOL_TYPE_STYLUS: we append the string "(stylus, " to `touchTypeDescription`. We
         * then initialize `float stylusPressure` with the current pressure of `e` of
         * its first pointer index, and then append the string formed by concatenating the string
         * "pressure: " and the string value of `stylusPressure` to `touchTypeDescription`.
         ...
         * pressed generated by our method `getButtonsPressed` from `e` to
         * `touchTypeDescription`. Finally we append the string ")" to `touchTypeDescription`
         * and break.
         *
         *  *
         * TOOL_TYPE_ERASER: we append the string "(eraser)" to `touchTypeDescription` and
         * break.
         *
         *  *
         * TOOL_TYPE_MOUSE: we append the string "(mouse)" to `touchTypeDescription` and
         * break.
         *
         *  *
         * default: we append the string "(unknown tool)" to `touchTypeDescription` and
         * break.
         *
         *
         ...
         * @return a human-readable string describing the type of touch that triggered `MotionEvent e`.
         */
         ...
         * input MotionEvent occurred. We initialize our variable `String buttons` with the empty
         * string, then if `e` has BUTTON_PRIMARY pressed we append the string " primary" to
         ...
         * @return a human-readable string listing all the stylus buttons that were pressed during our
         * parameter `MotionEvent e`
         */
        @TargetApi(21)
