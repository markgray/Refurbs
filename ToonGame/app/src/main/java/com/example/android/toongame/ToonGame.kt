...
class ToonGame : ComponentActivity() {
     ...
     * argument a lambda that accepts the [View] passed the lambda in variable `v`
     * and the [WindowInsetsCompat] passed the lambda in variable `windowInsets`.
     * It initializes its [Insets] variable `insets` to the [WindowInsetsCompat.getInsets]
     * of `windowInsets` with [WindowInsetsCompat.Type.systemBars] as the argument, then
     * it updates the layout parameters of `v` to be a [ViewGroup.MarginLayoutParams] with
     * the left margin set to `insets.left`, the right margin set to `insets.right`, the
     * top margin set to `insets.top`, and the bottom margin set to `insets.bottom`.
